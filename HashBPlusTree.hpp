...
//            puts("4");
        ...
        void BPFind(int cur, const std::string&key,vector<std::pair<int,long long>>&res) {
            //找一数列可行的
            unsigned long long Hkey=stringHash(key);
            Key defaultKey = std::make_pair(Hkey, -9223372036854775808ll);
            innerNode curNode(memoInner->read(cur));
            ...
            if (curNode.childIsLeaf) {
                ...
                if (target.leafKey[pos].first == Hkey) {
                    while (target.leafKey[pos].first == Hkey) {
    ...
    public:
        ...
        void insert(const std::string &fir,const long long&sec, const data &val) {
            Key key(std::make_pair(stringHash(fir),sec));
//        	puts("BPlusTree insert here");
        ...
        bool remove(const std::string &fir, const long long& sec) {
            Key key(std::make_pair(stringHash(fir),sec));
        ...
        void find(const string&key,vector<std::pair<int,long long>>&res){
            ...
            if (rootNode.childIsLeaf) {
                ...
                if (target.leafKey[pos].first == Hkey) {
                    while(target.leafKey[pos].first == Hkey&&pos<target.sum){
                        res.push_back(std::make_pair(target.leafData[pos],target.leafKey[pos].second));
                        pos++;
